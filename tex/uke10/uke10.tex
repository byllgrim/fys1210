Eget kompendium om digitale kretser.

\subsection{Kretsfamilier (Logic families)}
  \input{./tex/uke10/kretsfamilier}

  \subsubsection{NMOS}
    \input{./tex/uke10/nmos}

  \subsubsection{DTL}
    \input{./tex/uke10/dtl}

  \subsubsection{TTL}
    \input{./tex/uke10/ttl}

\subsection{Kombinatoriske kretser}
  \input{./tex/uke10/kombinatorisk}

  \subsubsection{Binær addisjon}
    \input{./tex/uke10/addisjon}

  \subsubsection{Adders}
    \input{./tex/uke10/adders}

\subsection{Sekvensielle kretser}
    \input{./tex/uke10/sekvensiell}

  \subsubsection{Dekoder / Enkoder}
    \input{./tex/uke10/koder}

  \subsubsection{ROM}
    \input{./tex/uke10/rom}

  \subsubsection{Latches}
    \input{./tex/uke10/latches}
